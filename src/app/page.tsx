...
export default function Home() {
  ...
  return (
    <div ref={containerRef} className="flex flex-col gap-24 opacity-0 transition-opacity duration-700 ease-in-out" style={{ opacity: 1 }}>
      {/* Hero Section */}
      <section className="flex flex-col gap-10">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          ...
          <div className="flex flex-col gap-3 items-start sm:items-end">
             <div className="flex items-center gap-2 bg-muted/30 px-3 py-1.5 rounded-full border border-border">
              <span className="text-xs font-mono uppercase tracking-wider text-muted-foreground">Currently building @doodlzz</span>
            </div>
        ...
        <div className="flex flex-col gap-8 max-w-3xl">
          <p className="hero-bio text-base leading-relaxed text-muted-foreground">
            I create minimalist and <span className="text-foreground font-semibold underline decoration-border underline-offset-8">performant</span> web experiences. 
            Currently obsessed with design systems, animations, and everything in between. 
            I build interactive apps using <span className="inline-flex items-center gap-1.5 bg-muted/40 border border-border px-2 py-1 rounded-md text-xs font-semibold"><img src="https://skillicons.dev/icons?i=nextjs" className="w-4 h-4" /> Next.js</span>, <span className="inline-flex items-center gap-1.5 bg-muted/40 border border-border px-2 py-1 rounded-md text-xs font-semibold"><img src="https://skillicons.dev/icons?i=ts" className="w-4 h-4" /> TypeScript</span> and <span className="inline-flex items-center gap-1.5 bg-muted/40 border border-border px-2 py-1 rounded-md text-xs font-semibold"><img src="https://skillicons.dev/icons?i=react" className="w-4 h-4" /> React</span>.
          </p>
          ...
          <div className="flex flex-wrap gap-4 items-center">
            ...
            <div className="flex items-center gap-3 ml-2">
              {[
                ...
                { icon: Linkedin, href: "https://www.linkedin.com/in/adeebiqbalkhan/" }

              ].map((social, i) => (
                <Link key={i} href={social.href} target="_blank" className="p-2 text-muted-foreground hover:text-foreground transition-all hover:bg-muted/30 rounded-xl border border-transparent hover:border-border">
      ...
      <section className="flex flex-col gap-10">
        <div className="flex items-center gap-4">
          <div className="badge">Technical Skills</div>
          <h2 className="text-2xl font-bold text-foreground">My Toolkit</h2>
        </div>
        <div className="flex flex-wrap gap-3">
          {["js", "ts", "nodejs", "express", "react", "nextjs", "mongodb", "mysql", "postgres", "tailwind", "bootstrap", "prisma", "websocket", "docker", "aws", "git", "github", "postman", "figma", "wordpress"].map(tech => (
            <div key={tech} className="group flex items-center gap-2.5 bg-card/40 border border-border/50 px-4 py-2.5 rounded-2xl hover:bg-card/70 transition-all duration-300 glass hover:scale-105">
              <img src={`https://skillicons.dev/icons?i=${tech}`} alt={tech} className="w-5 h-5 grayscale group-hover:grayscale-0 transition-all duration-500" />
              <span className="text-[11px] font-bold capitalize tracking-tight">{tech}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="flex flex-col gap-10">
        <div className="flex items-center gap-4">
          <div className="badge">Journey</div>
      ...
      <section className="flex flex-col gap-10 pt-10 pb-6" id="contact">
        <div className="flex items-center gap-4">
          <div className="badge">Connect</div>
        ...
        <div className="relative group">
          ...
          <div className="relative p-10 bg-card/40 border border-border/50 rounded-[2.5rem] glass flex flex-col gap-8 text-center items-center">
            <div className="max-w-md flex flex-col gap-4">
              <h3 className="text-3xl font-black tracking-tight text-foreground">Let&apos;s build something <span className="text-muted-foreground italic">extraordinary</span>.</h3>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Link 
                href="mailto:adeeb.codes@gmail.com"
            ...
            <div className="flex items-center gap-6 pt-4 border-t border-border/30 w-full justify-center">
              {[
                { icon: Twitter, href: "https://x.com/adeeebik", label: "Twitter" },
                { icon: Github, href: "https://github.com/adeebik", label: "GitHub" },
                { icon: Linkedin, href: "https://www.linkedin.com/in/adeebiqbalkhan/", label: "LinkedIn" }
              ].map((s) => (
                <Link key={s.label} href={s.href} target="_blank" className="flex items-center gap-2 text-xs font-bold text-muted-foreground hover:text-foreground transition-colors">
      ...
      <footer className="pt-12 border-t border-border flex flex-col gap-8">
        <p className="text-[10px] text-center font-bold uppercase tracking-widest text-muted-foreground/40">
          © {new Date().getFullYear()} Adeeb Iqbal Khan. Built with love and a lot of caffeine.
        </p>
      </footer>
